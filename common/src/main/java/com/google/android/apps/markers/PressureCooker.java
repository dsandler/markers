...
public class PressureCooker {
    ...
    public PressureCooker(Context context) {
        this(context, DEF_PRESSURE_MIN, DEF_PRESSURE_MAX);
    }

    public PressureCooker(Context context, float pressureMin, float pressureMax) {
        mContext = context;
        mDebugPaint.setTextSize(9 * context.getResources().getDisplayMetrics().density);
        loadStats(pressureMin, pressureMax);
    }
    
    public void loadStats(float pressureMin, float pressureMax) {
        SharedPreferences prefs = mContext.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE);

        mPressureMin = prefs.getFloat(PREF_PRESSURE_MIN, pressureMin);
        mPressureMax = prefs.getFloat(PREF_PRESSURE_MAX, pressureMax);
    ...
    public void drawDebug(Canvas canvas, float x, float y) {
